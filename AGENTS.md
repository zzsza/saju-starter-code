...
- 새 기능을 시작할 때 다음 번호의 Spec을 `docs/specs/`에 만들고 `docs/status.md`에 체크되지 않은 항목으로 추가합니다. 예: `- [ ] 001-feature-name.md`
...
- 코드를 생성하거나 변경하면, 문서·주석·서식만 바뀐 경우를 제외하고 구현을 맡지 않은 서브에이전트에게 관련 Test 작성·보강과 실행을 위임합니다. 주 에이전트는 결과를 검토하고 최종 검증을 담당합니다.
- Test가 실패하면 작업을 완료로 처리하지 않습니다. 실패 원인이 구현인지 Test인지 확인해 수정하고, 관련 Test가 모두 통과할 때까지 구현과 검증을 반복합니다.
...
- 커밋 메시지는 변경 내용을 알 수 있는 한국어로 작성합니다.
- 불필요한 대규모 리팩터링을 피합니다.
